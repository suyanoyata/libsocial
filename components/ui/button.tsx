...
const buttonVariants = cva(
  "p-2.5 px-4 rounded-lg items-center dark:shadow-none justify-center flex-row gap-1 rounded-full",
  {
    variants: {
      variant: {
        link: "",
        outline: "dark:border-violet-100 border-violet-700 border-2 bg-transparent",
        ...
        ghost: "bg-transparent active:bg-violet-100 dark:active:bg-violet-400/20",
        tonal: "bg-violet-200 dark:bg-violet-950 active:bg-violet-100 dark:active:bg-violet-900",
      },
      size: {
        default: "py-3",
        sm: "py-2 px-3",
    ...
    return (
      ...
      >
        {typeof iconLeft === "string" ? (
          <Icon name={iconLeft} strokeWidth={2.4} className={cn(textVariants({ variant }))} />
        ) : (
          iconLeft
        )}
        {asChild ? (
        ...
        {typeof iconRight === "string" ? (
          <Icon name={iconRight} strokeWidth={2.4} className={cn(textVariants({ variant }))} />
        ) : (
          iconRight
        )}
      </Pressable>
