...
export const bookmarkRouter = t.router({
  list: t.protected.input(z.string().optional()).query(async ({ input, ctx }) => {
  ...
  update: t.protected.input(updateBookmarkSchema).mutation(async ({ input, ctx }) => {
    return await bookmarksService.updateBookmark(ctx.user.id, input)
  }),
  delete: t.protected.input(deleteBookmarkSchema).mutation(async ({ input, ctx }) => {
