...
export const BookmarkEventsProvider = ({ children }: BookmarkEventsProps) => {
  ...
  useEffect(() => {
    DeviceEventEmitter.addListener(BookmarkEvents.CREATE_BOOKMARK, (slug_url: string) => {
    ...
    DeviceEventEmitter.addListener(BookmarkEvents.UPDATE_READ_BOOKMARK, async (data) => {
    ...
    DeviceEventEmitter.addListener(BookmarkEvents.UPDATE_WATCH_BOOKMARK, async (data) => {
      const bookmark = client.getQueryData(
        trpc.bookmarks.get.queryKey({
          slug_url: data.slug_url,
          type: "anime"
    ...
    DeviceEventEmitter.addListener(BookmarkEvents.DELETE_BOOKMARK, (slug_url: string) => {
      client.invalidateQueries({
        queryKey: trpc.bookmarks.list.queryKey()
      })
    ...
    return () => {
      ...
      DeviceEventEmitter.removeAllListeners(BookmarkEvents.UPDATE_WATCH_BOOKMARK)
